...
NumericMatrix dmatC(DataFrame T) {
  ...
  for (i1=0;i1<(rows-1);i1++){
    for (i2=(i1+1);i2<rows;i2++){
      for (j=0;j<cols;j++){
        List COL = T[j];
        DataFrame ROW1=Rcpp::as<DataFrame>(COL[i1]);
        DataFrame ROW2=Rcpp::as<DataFrame>(COL[i2]);
  ...
  for(i=0;i<n;i++){
    for(clu=0;clu<k;clu++){
      for(j=0;j<p;j++){
        List COL = Tib[j];
        List COLc = Cen[j];
        DataFrame ROW1=Rcpp::as<DataFrame>(COL[i]);
        DataFrame ROW2=Rcpp::as<DataFrame>(COLc[clu]);
        NumericVector a = ROW1[0];
        NumericVector wa = ROW1["freq"];
        NumericVector b = ROW2[0];
        NumericVector wb = ROW2["freq"];
